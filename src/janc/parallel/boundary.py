...
def boundary_conditions(U,aux):
    ...
    U_with_lb,aux_with_lb = jax.lax.cond(device_idx==0,lambda:boundary_func['left_boundary'](U_periodic_pad,aux_periodic_pad),lambda:(U_periodic_pad,aux_periodic_pad))
    U_with_rb,aux_with_rb = jax.lax.cond(device_idx==(num_devices-1),lambda:boundary_func['right_boundary'](U_with_lb,aux_with_lb),lambda:(U_with_lb,aux_with_lb))
